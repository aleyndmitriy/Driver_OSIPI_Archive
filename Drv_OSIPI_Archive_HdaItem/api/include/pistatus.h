...
$Archive: /PI/3.4.370/libraries/piut/pistatus.h $
$Revision: 163 $     $Date: 1/14/10 8:49p $

--------------------------------------------------------------------------
Copyright (c) 1994-2009 OSIsoft, LLC. All rights reserved.

THIS SOFTWARE CONTAINS CONFIDENTIAL INFORMATION AND TRADE SECRETS OF
OSIsoft, LLC.  USE, DISCLOSURE, OR REPRODUCTION IS PROHIBITED
WITHOUT THE PRIOR EXPRESS WRITTEN PERMISSION OF OSIsoft, LLC.
...
enum {
   ...
   /* -10249 <= PI_ERR_??? <= -10000 */      /* generic errors */
   ...
   /* -10249 <= PI_AUTHINFO_??? <= -10225 */
   PI_AUTHINFO_NOTEXIST          = -10225, /* AuthInfo does not exist */
   PI_AUTHINFO_INVTYPE           = -10226, /* AuthInfo type is invalid */
   PI_AUTHINFO_TYPE_REQUIRED     = -10227, /* AuthInfo type is required */
   PI_AUTHINFO_TYPE_ID_MISMATCH  = -10228, /* AuthInfo type/identifier mismatch */
   PI_AUTHINFO_ID_REQUIRED       = -10229, /* AuthInfo identifier is required */

   /* -10299 <= PI_ACL_??? <= -10250 */
   PI_ACL_ACE_EMPTY           = -10250, /* ACL is invalid: ACE is empty */
   PI_ACL_ACE_NOIDENT         = -10251, /* ACL is invalid: ACE requires an existing Identity */
   PI_ACL_ACE_INVTYPE         = -10252, /* ACL is invalid: ACE type is missing or invalid */
   PI_ACL_ACE_INCVERBLIST     = -10253, /* ACL is invalid: ACE security verb list is incomplete */
   PI_ACL_ACE_VERBMISSING     = -10254, /* ACL is invalid: ACE security verb is missing */
   PI_ACL_ACE_INVVERB         = -10255, /* ACL is invalid: ACE security verb is invalid */
   PI_ACL_ACE_UNEXPCONTENT    = -10256, /* ACL is invalid: additional ACE content is unexpected */
   PI_ACL_ACE_INVIDENT        = -10257, /* ACL is invalid: ACE Identity is invalid */
   PI_ACL_ACE_INVPRIVS        = -10258, /* ACL is invalid: ACE privileges are invalid */
   PI_ACL_ACESCONFLICT        = -10259, /* ACL is invalid: conflicting ACEs are not supported */

   /* -10349 <= PI_IDENT_??? <= -10300 */
   PI_IDENT_INVNAME              = -10300,  /* Identity name is invalid: cannot be empty, contain chars '|' and ':', or be a positive integer */
   PI_IDENT_INVTYPE              = -10301,  /* Identity type is invalid */
   PI_IDENT_NOTYPEEDIT           = -10302,  /* Identity type cannot be edited */
   PI_IDENT_INVFLAGS             = -10303,  /* Identity config flags are invalid */
   PI_IDENT_INVFLAGS_4PIADMIN    = -10304,  /* piadmin config flags are invalid */
   PI_IDENT_INVFLAGS_4PIWORLD    = -10305,  /* PIWorld config flags are invalid */
   PI_IDENT_INVFLAGS_4STUB       = -10306,  /* Built-in identity config flags are invalid */
   PI_IDENT_DISABLED             = -10307,  /* Identity is disabled */
   PI_IDENT_DISABLED_4TRUST      = -10308,  /* Identity is disabled for Trusts */
   PI_IDENT_DISABLED_4MAPPING    = -10309,  /* Identity is disabled for IdentityMappings */
   PI_IDENT_NOTEXIST             = -10310,  /* Identity does not exist */
   PI_IDENT_DISABLED_4EXPLICIT   = -10311,  /* Identity is disabled for explicit login */
   PI_IDENT_INVFLAGS_4PIADMINS   = -10312,  /* piadmins config flags are invalid */
   PI_IDENT_INVFLAGS_4PIUSERS    = -10313,  /* piusers config flags are invalid */
   PI_IDENT_NOEDIT_4DYNUSER      = -10314,  /* A dynamically created PI User cannot be modified */

   /* -10399 <= PI_IDMAP_??? <= -10350 */
   PI_IDMAP_NOTEXIST          = -10350,  /* IdentityMapping does not exist */
   PI_IDMAP_INVTYPE           = -10351,  /* IdentityMapping type is invalid */
   PI_IDMAP_NOTYPEEDIT        = -10352,  /* IdentityMapping type cannot be edited */
   PI_IDMAP_INVFLAGS          = -10353,  /* IdentityMapping flags are invalid */
   PI_IDMAP_IDENTREQUIRED     = -10354,  /* IdentityMapping requires an existing Identity */
   PI_IDMAP_PRINCIPREQUIRED   = -10355,  /* IdentityMapping requires a unique security principal */
   PI_IDMAP_CACHE_NOTAVAIL    = -10356,  /* IdentityMapping cache is not available */
   PI_IDMAP_CACHE_INVOPER     = -10357,  /* IdentityMapping cache operation is invalid */
   ...
   PI_XS_NOTADMIN     = -10417, /* User must be piadmin or a member of piadmins for this operation */
   PI_XS_DBSECNOTINIT   = -10418, /* DB Security not initialized */
   PI_XS_NULLSECOBJ = -10419, /* Unable to retrieve secure object */
   PI_XS_NOWRITESECONDARY           = -10420, /* Direct write operations disallowed on secondary server of collective*/
   PI_XS_SECOBJ_ACLUSEUNEXPECTED    = -10421, /* Secure object: version is not supported - downgrade required */
   PI_XS_SECOBJ_ACLUSEEXPECTED      = -10422, /* Secure object: version is not supported - upgrade required   */
   PI_XS_SECOBJ_NOACLINCACHE        = -10423, /* Secure object: associated ACL not found in cache */
   PI_XS_SECOBJ_SPECAMBIGUOUS       = -10424, /* Secure object: spec is ambiguous - use either the Security attribute OR all the Owner, Group, Access attributes */
   PI_XS_SECOBJ_NOTCOMPATIBLE       = -10425, /* Secure object: spec requires use of Security attribute exclusively */
   PI_XS_SECOBJ_PTSPEC_NOTCOMPAT    = -10426, /* Secure object: spec requires use of PtSecurity attribute exclusively */
   PI_XS_SECOBJ_DATASPEC_NOTCOMPAT  = -10427, /* Secure object: spec requires use of DataSecurity attribute exclusively */
   PI_XS_SECOBJ_PTSPEC_AMBIGUOUS    = -10428, /* Secure object: spec is ambiguous - use either the PtSecurity attribute OR all the PtOwner, PtGroup, PtAccess attributes */
   PI_XS_SECOBJ_DATASPEC_AMBIGUOUS  = -10429, /* Secure object: spec is ambiguous - use either the DataSecurity attribute OR all the DataOwner, DataGroup, DataAccess attributes */
   PI_XS_UNSUPPORTEDTOKEN           = -10430, /* Security token is not supported - mismatched client / server version */
   PI_XS_AUTHMETHDISABLED           = -10431, /* Authentication method is disabled by current server policy */
   PI_XS_DBSEC_BUILTIN_NODELETE     = -10432, /* Built-in DBSecurity entry cannot be deleted */
   PI_XS_NOMAPPING                  = -10433, /* No identity mapping for this request */
   PI_XS_DBSEC_EDIT_INPROGRESS      = -10434, /* Previous edit to DBSecurity entry is still pending */
   PI_XS_FAILED_AUTH_RECORD         = -10435, /* Failed to acquire authentication identifier */
   PI_TRUST_INVFLAGS                = -10436, /* Trust flags are invalid */
   ...
   PI_FB_FILEPATH_TOOLONG  = -10479,   /* File path and name exceed 300 characters */
   PI_FB_UPGRADE_REQUIRED  = -10480,   /* File version is not supported - upgrade required */
   ...
   PI_PD_PTCLSNOCOMCHNG = -10596, /* Disallow Changes in point class affecting com connector status */
   PI_PD_RECNOMISMATCH = -10597, /* Recno Mismatch on secondary */
   PI_PD_ATTRIBUTEINUSE = -10598, /* Attribute in use. Cannot remove from a set or class */
   PI_PD_DUPLICATE_RECNO = -10599, /* Multiple points detected with the same recno */
   ...
   PI_DS_INVALIDSETNO      = -10709, /* Event does not match point digital set */
   PI_DS_NOTADIGITALPOINT  = -10710, /* Digital event was received for a non-digital point */
   ...
   PI_AR_LOWDISKSPACE   = -11125, /* Low disk space */
   ...
   PI_AR_RECNOMISMATCH  = -11139, /* Point recno mismatch */
   PI_AR_MAXQUERYEXEC   = -11140, /* Archive query exceeded maximum execution time (see Archive_MaxQueryExecutionSec) */
   PI_AR_RECLOADCOUNT = -11141, /* Too many events on record load - most likely a corruption */
   ...
   PI_RDR_NOARCSIGNUP         = -11210,   /* PI-Redirector does not support signup for archive updates */
   ...
   /* buffer subsystem/buffered snap specific */
   /* -11599 <= PI_BUFSS <= -11400, */
   PI_BUFSS_FIRST             = -11400,
   PI_BUFSS_CREATEBUFMGR      = -11400,  /* Failed to initialize buffer manager class */
   PI_BUFSS_CREATEPTERRHASH   = -11401,  /* Unable to create point error hash table */
   PI_BUFSS_CREATEMEMFILEBUF  = -11402,  /* Error creating shared memory buffers or disk buffer file */
   PI_BUFSS_LOWDISKSPACE      = -11403,  /* Insufficient disk space for maximum buffer size */
   PI_BUFSS_SHUTDOWNSIGNAL    = -11404,  /* BufServ shutdown signal received */
   PI_BUFSS_NODATA            = -11405,  /* No data from API processes */
   PI_BUFSS_INVALIDSRVID      = -11406,  /* Invalid server ID while loading buffer table */
   PI_BUFSS_INVALIDHOST       = -11407,  /* Impossible connection, invalid hostname */
   PI_BUFSS_SESSIONFAILED     = -11408,  /* Error opening remote session */
   PI_BUFSS_BADHEADERCODE     = -11409,  /* Error loading snapshot table header */
   PI_BUFSS_UNKOWNREGID       = -11410,  /* Invalid registration ID from buffered source */
   PI_BUFSS_DUPLICATEREGID    = -11411,  /* Duplicate ID when on buffered source registration */
   PI_BUFSS_SESSIONRUNNING    = -11412,  /* Buffer session is running */
   PI_BUFSS_NOCLIENTINI       = -11413,  /* Can't find PICLIENT.INI */
   PI_BUFSS_EVENTBUFPOINT     = -11414,  /* Buffered point does not accept new events */
   PI_BUFSS_PISYSQUERY        = -11415,  /* Error getting system information from PI Server */
   PI_BUFSS_EMPTYBSL          = -11416,  /* Empty or invalid BUFFEREDSERVERLIST in PICLIENT.INI */
   PI_BUFSS_BUFFERINGOFF      = -11417,  /* Buffering is turned off, process exiting */
   PI_BUFSS_1SERVERONLY       = -11419,  /* Buffering is supported for only one PI Server or Collective */
   PI_BUFSS_BSLVALIDATION     = -11420,  /* Error validating first-time configuration, no server connection */
   PI_BUFSS_BADSERVERVERSION  = -11421,  /* Remote PI Server is not compatible (minimum version: 3.4.375) */
   PI_BUFSS_PINSNODEONLY      = -11422,  /* Buffer Subsystem is not configured to run on PI Server nodes */
   PI_BUFSS_NOTREGISTERED     = -11423,  /* Buffer subsystem is not registered with snapshot */
   PI_BUFSS_UNREGISTERFAILED  = -11424,  /* Error while unregistering from remote snapshot */
   PI_BUFSS_INVALIDSRVCFG     = -11425,  /* Unable to retrieve server configuration, please check PI trust definition */
   PI_BUFSS_SESSIONNOTRUNNING = -11426,  /* Buffer session is not running */
   PI_BUFSS_THREADNOTSTOPPING = -11427,  /* Session thread did not respond to shutdown signal */
   PI_BUFSS_INVALIDSESSION    = -11428,  /* Buffer session is invalid */
   PI_BUFSS_OOOEVSKIPPED      = -11429,  /* Out-of-order event discarded from buffered source */
   PI_BUFSS_INVALIDAPIEVENT   = -11430,  /* Unexpected API event from shared-memory buffer */
   PI_BUFSS_TRACENODATA       = -11431,  /* Trace has no data in buffer */
   PI_BUFSS_TRACEERROR        = -11432,  /* Trace is not started or timed out */
   PI_BUFSS_INVALIDCTRLCMD    = -11433,  /* Unknown or invalid control command */
   PI_BUFSS_SESSIONBADSTATE   = -11434,  /* Invalid buffer session state */
   PI_BUFSS_LAST              = -11599,
   ...
   PI_OFFL_ABORTED      = -11911, /* Operation aborted by user */
   ...
   PI_UPD_MMQWRITEFAILED = -12157, /* Failed to add new event to MMQ */
   ...
   PILIC_LICEXCEEDED	= -12206, /* usage exceeded licensed amount */
   ...
   PILIC_ANONYMOUSAPP = -12230,     /* Maximum licensed Anonymous Application connections exceeded. Connection refused.*/ 
   PILIC_ANONYMOUSSDK = -12231,     /* Maximum licensed Anonymous SDK Application connections exceeded. Connection refused.*/ 
   PILIC_ANONYMOUSAPI = -12232,     /* Maximum licensed Anonymous API Application connections exceeded. Connection refused.*/ 
   PILIC_ERRGETCLUS = -12233,       /* Error getting Cluster info. */
   PILIC_POINTSOURCE = -12234,       /* Disallowed Point Source */
   PILIC_PTSOURCECOUNT = -12235,       /* Exceeded Point Count allowance for Point-Source */
   //PLI19600OSI8
   PILIC_NOTINCLUDED_CLIENTAPP = -12236, /* This client application is not explicitly allowed in the license file. */
   PILIC_NOTINCLUDED_INTERFACE = -12237, /* This interface is not explicitly allowed in the license file. */
   PILIC_NOTINCLUDED_MIDDLEWARE = -12238, /* This middle-ware application is not explicitly allowed in the license file. */
   PILIC_NOTINCLUDED_SERVERAPP = -12239, /* This server or UDS application is not explicitly allowed in the license file. */
   PILIC_NOTINCLUDED_3RDPARTY = -12240, /* This application is not explicitly allowed in the license file. */  
   PILIC_3RDPARTY = -12241, /* Not licensed to use this application. */  
   ...
   /* PI Generic Table Errors */
   /* -12400 through -12499	  */
   PI_GTB_COLLECTION_UNINITIALIZED = -12400, /*Generic Table: Collection Uninitialized */
   PI_GTB_UNKNOWN_TABLE = -12401, /* Generic Table: Unknown Table */
   PI_GTB_TABLE_UNINITIALIZED = -12402, /* Generic Table: Table Uninitialized */
   PI_GTB_DUPLICATE_TABLE = -12403, /* Generic Table: Table Definition Already Exists */
   PI_GTB_INVALID_TABLE_OPERATION = -12404, /* Generic Table: Invalid Operation */
   PI_GTB_RECID_MISMATCH = -12405, /* Generic Table: RecordID mismatch */
   PI_GTB_RECNAME_MISMATCH = -12406, /* Generic Table: RecordName mismatch */
   PI_GTB_NO_RECID_OR_RECNAME = -12407, /* Generic Table: RecordID or RecordName must be specified */
   ...
   PI_BDBNOTACCESSIBLEONSECONDARY = -16030, /* Batch database access disabled on secondary server of collective */
   ...
   PI_BACKUP_INVALID_COMPONENT_NAME              = -16842,    /* Invalid component name or archive number. */
   PI_BACKUP_COPY_FILE_FAILED                    = -16843,    /* Copy file failed for backup operation */
   PI_BACKUP_VSS_API_CALL_FAILED                 = -16844,    /* VSS API Callled failed */
   PI_BACKUP_FILE_BACKUPSTAT_UNINITIALIZED       = -16845,    /* File not copied during backup */
   PI_BACKUP_ARCHK_FAILED                        = -16846,    /* Backup verification pidiag -archk failed */
   PI_BACKUP_OF_COMPONENT_FAILED                 = -16847,    /* Backup of a component failed for a VSS, component-mode backup */
   PI_BACKUP_VERIFYDB_FAILED                     = -16848,    /* Backup verification pibasess -verifydb failed */
   PI_BACKUP_NO_BACKEDUP_TIMES_UPDATED           = -16849,    /* No backup times were updated for any files */
   PI_BACKUP_TIME_VERIFICATION_FAILED            = -16850,    /* Last backup time verification check failed for file */
   PI_BACKUP_COMMIT_FAILED                       = -16851,    /* Error moving files from pre-commit directory to the target directory of the backup */
   PI_BACKUP_BACKEDUP_TIME_NOT_UPDATED           = -16852,    /* Backed up time not updated for file */
   PI_BACKUP_PRECOMMIT_COMMAND_FAILED            = -16853,    /* Backup precommit custom command failed */
   PI_BACKUP_SUMMARY_RETRIEVAL_FAILED            = -16854,    /* Retrieval of PI Backup Summary failed */
   PI_BACKUP_INVALID_TYPE                        = -16855,    /* Invalid backup type */
   PI_BACKUP_VERIFYSNAPDB_FAILED                 = -16856,    /* Backup verification of the snapshot database failed */
   PI_BACKUP_UNKNOWN_STATUS                      = -16857,    /* The backup completed, but the status is unknown */
   ...
   PI_BACKUP_VSS_FAILURE                              = -16911,    /* The backup failed due to a failure in Volume Shadow Copy Services (VSS) */ 
   PI_BCKSTATE_BACKUP_PENDING                         = -16912,    /* Backup pending start */ 
   PI_BACKUP_PREPAREBACKUP_FAILED                     = -16913,    /* PrepareBackup failed */
   PI_BACKUP_INVALID_STATE_TRANSITION                 = -16914,    /* Invalid state transition for backup */
   PI_BACKUP_ABORTED                                  = -16915,    /* Backup was aborted */
   PI_BACKUP_SHUTDOWN_NOCOMPLETE                      = -16916,    /* Backup shutdown without completing */
   PI_BCKSTATE_BACKUP_IN_PROGRESS                     = -16917,    /* Backup in progress */
   PI_BCKSTATE_BACKUP_NOT_IN_PROGRESS                 = -16918,    /* Backup is not in progress */
   PI_BACKUP_FAILED                                   = -16919,    /* Backup of one or more components failed */ 
   ...
   PI_BCKRPC_INVALID_BACKUP_HISTORY_COMMAND           = -16924,    /* Invalid backup history command */
   ...
   /* Backup argument errors (invalid command lines) */
   PI_BACKUPARG_MISSING_FLAG_FOR_CRF_COMMAND          = -16950,    /* Missing flag. The -output is required and at least one of the -NewPIDir and -NewArcDir flags are required for -crf command */
   PI_BACKUPARG_MISSING_FLAG_FOR_SIMULATEVSS_COMMAND  = -16951,    /* Missing flag. The -PrepareBackup2Freeze, -Thaw2PostSnapshot, or -BackupShutdown flags must be specified after the -SimulateVSS flag */
   PI_BACKUPARG_MISSING_FLAG_FOR_TEST_COMMAND         = -16952,    /* Missing flag. The -freeze, -thaw, or -slowcopy flags must be specified along with the -backup -test command */
   ...
   PI_BCKLOCK_UNBALANCEDLOCKRETURN                     = -16986,   /* Unbalanced lock return for master backup lock */
   ...
   */
   ...
   PI_SYS_STATUSUNKNOWN                    = -18000,
   PI_SYS_MISSINGUPDATE                    = -18001,
   PI_SYS_BADUPDATE                        = -18002,
   PI_SYS_NOPRODUCER                       = -18003,
   PI_SYS_UNABLETOUNREGISTERPRODUCER       = -18004,
   PI_SYS_NOACTIVE                         = -18005,
   PI_SYS_NOPRIMARY                        = -18006,
   PI_SYS_DISCONNECTED                     = -18007,
   PI_SYS_UNABLETOSIGNUPFORUPDATES         = -18008,
   PI_SYS_UNABLETOUNSIGNUPFORUPDATES       = -18009,
   PI_SYS_UNABLETODISCONNECTFROMPRIMARY    = -18010,
   PI_SYS_NOCOLLECTIVE                     = -18011,
   PI_SYS_INVALIDROLE                      = -18012,
   PI_SYS_MEMBERSERVERINERROR              = -18013,
   PI_SYS_UNABLETOREADSTATUSFROMPRIMARY    = -18014,
   PI_SYS_UNABLETOSENDUPDATES              = -18015,
   PI_SYS_UNABLETOSENDSTATUSTOPRIMARY      = -18016,
   PI_SYS_UNABLETOREADCHANGERECORDSFROMPRIMARY = -18017,
   PI_SYS_COMMTIMEOUT                      = -18018,
   PI_SYS_DELETED                          = -18019,
   PI_SYS_TIMEOUTOFSYNC                    = -18020,
   PI_SYS_FAILEDTOENABLEREPL               = -18021,
   PI_SYS_SYNCTIMEOUT                      = -18022,
   PI_SYS_UNABLETOSAVEUPDATES              = -18023,
   PI_SYS_UNABLETOLOADUPDATES              = -18024,
   PI_SYS_ORPHANSERVER                     = -18025,
   PI_SYS_UNABLETOCHANGEROLEOFONLINEPRIMARY = -18026,
   PI_SYS_NOTREADYTOPROCESSCHANGES         = -18027,
   PI_SYS_UNABLETOSIGNUPSECONDARYS         = -18028,
   PI_SYS_INVALIDATTRFORNONREPLICATINGSERVER = -18029,
   PI_SYS_INVALIDNAME                      = -18030,
   PI_SYS_INVALIDNAMELOCALHOST             = -18031,
   PI_SYS_INVALIDFQDN                      = -18032,
   PI_SYS_DUPLICATEFQDN                    = -18033,
   PI_SYS_UNABLETOPRODUCEUPDATE            = -18034,
   PI_SYS_UNABLETOREADUPDATE               = -18035,
   PI_SYS_UNABLETORENAMECURRENTSERVER      = -18036,
   PI_SYS_UNABLETORENAMEONLINESERVER       = -18037,
   PI_SYS_UNABLETOREMOVECURRENTSERVER      = -18038,
   PI_SYS_UNABLETOREMOVEONLINESERVER       = -18039,
   PI_SYS_CHGRECVERSUNSUPPORTED            = -18040, /* Unsupported replication change record version; upgrade may be required */
   PI_SYS_DUPLICATESERVERID                = -18041,
   PI_SYS_UNABLETOGETLICENSEFROMPRIMARY    = -18042,
   PI_SYS_LICENSEERROR                     = -18043,
   PI_SYS_LICENSESETTOEXPIRE               = -18044,
   PI_SYS_LICENSEEXPIRED                   = -18045,
   PI_SYS_NOFQDN                           = -18046,
   PI_SYS_NOIPADDR                         = -18047,
   PI_SYS_UNABLETOREMOVECOLLECTIVE         = -18048,
   PI_SYS_UNABLETOGETEVENTS                = -18049,
   PI_SYS_NOCRYPTO                        = -18050,

   /* Codes for external DLL interface
   */
   PI_EXT_ERROR       = -19000,  /* Generic error */
   PI_EXT_MOREDATA    = -19001,  /* Buffer too small */
   PI_EXT_NOTFOUND    = -19002,  /* Requested item not found */
   PI_EXT_INVALIDARG  = -19003,  /* NULL pointer argument */
   PI_EXT_SECPKGERROR = -19004,  /* Security package internal error */
   PI_EXT_BADHANDLE   = -19005,  /* Instance index out of range */
   PI_EXT_NOFUNCTION  = -19099,  /* Function not available */

   /* Codes for authentication and authorization
   */
   PI_AUTHN_NOOBJECT       = -19100,  /* Authentication object does not exist */
   PI_AUTHN_INVALIDMSGTYPE = -19101,  /* Invalid authentication message type */
   PI_AUTHN_NOHANDLE       = -19102,  /* Instance handle not available */
   PI_AUTHN_NOBUFFER       = -19103,  /* Payload buffer not available */
   PI_AUTHN_INVALIDAUTHRES = -19104,  /* inconsistent createsession result */
   ...
   PI_BACKUP_FILE_COPY_SKIPPED    = -30152,    /* File copy skipped because the source and target have the same file size and last modified date */
   PI_BACKUP_FILE_COPY_STATUS_UNKNOWN  = -30153,    /* File copy status is always unknown for a non-Component mode VSS backup */
   ...
   PI_GTB_ERRSENDREPLRECORD = -30240, /* Warning: Failed to send replication record */
   PI_GTB_ERRSENDAUDRECORD = -30241, /* Warning: Failed to send audit record */
   PI_UPD_OVERFLOW   = -30250,    /* Update queue overflow, some events missing */ 
   PI_UPD_GETEVENTSTHRESHOLD = -30251, /* MMQ cons getevents request exceeded size limit and more events remain in the queue */
   ...
   PI_AR_AUTODYNCONVERT = -30405, /* Fixed-size archive was automatically converted to dynamic type */
   PI_AR_RECNOALREADYUSED = -30406, /* Recno already used by same point */
   ...
   PI_CHAINED_SUBSYS_BLOCK_WARNING = -30503, /* Chained subsystem block did not complete successfully */
   PI_CHAINED_SUBSYS_BLOCK_INPROGRESS = -30504, /* Another chained subsystem block is already in progress */

   /* Warnings for license manager */
   PILIC_LICREACHED	= -30600, /* usage reaches licensed amount, i.e., the left amount is 0 */
   PILIC_NEWLICFILE = -30601, /* this is a new license file, i.e., the first time using this license file */
   PILIC_LICEXCEEDED_WARNING = -30602, /* usage exceeded licensed amount */
   PILIC_BADUSERKEYWARNING	= -30603  /* user key mismatch warning */
};
